...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    ...
    label = 0 if attack_type is None else 1  # Normal traffic has label 0, attack traffic has label 1

    for _ in range(count):
        tcp_flags = ''
        icmp_type = None
        interval = 0.4  # Default interval for normal traffic

        # Create the packet based on the attack type or normal traffic
        if icmp_type:
            pkt = IP(src=src_device.IP(), dst=dst_ip) / ICMP(type=icmp_type)
            
        elif attack_type is None:
            # Use UDP for normal traffic
            pkt = IP(src=src_device.IP(), dst=dst_ip) / UDP(sport=random.randint(1024, 65535), dport=dst_port)
        else:
            # Use TCP for attack traffic
            pkt = IP(src=src_device.IP(), dst=dst_ip) / TCP(sport=random.randint(1024, 65535), dport=dst_port, flags=tcp_flags)
...
def gather_traffic_data(net):
    # Function to simulate and gather network traffic data
    traffic_features = []  # List to store traffic data as feature vectors
    last_packet_time = {}  # Dictionary to store the last packet time for each host

    for host in net.hosts:  # Loop through each host in the network
        src_ip = host.IP()
        dst_ip = "10.0.0.1"
        ...
        packet_size = sum(packet_sizes)  # Sum of individual packet sizes for total size

        # Simulate traffic type based on random chance
        tcp_flags = ""
        icmp_type = None
        rst_count = 0
        label = 0  # Label for normal traffic

        current_time = time.time()
        if src_ip in last_packet_time:
            interval = current_time - last_packet_time[src_ip]
        else:
            interval = random.uniform(0.1, 1)  # Initial interval can be randomized
        ...
        feature = TrafficFeature(src_ip=src_ip, dst_ip=dst_ip, src_port=src_port, dst_port=dst_port,
                                 ...
                                 tcp_flags=tcp_flags, icmp_type=icmp_type, rst_count=rst_count, label=label)
...
async def main():
    ...
    task1 = asyncio.create_task(generate_traffic(net, 'h1', '10.0.0.1', 502, 6250))  # Normal UDP traffic
    # Monitoring network in parallel, not meant to be terminated by packet generation count
