...
class DDoSDetector:
    ...
    def detect_ddos(self, labels):
        ...
        for cluster, features in traffic_clusters.items():
            ...
            socket_conditions = [len(src_ips) >= 2, active_connections >= 20, zero_window_packets > 10]
...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    ...
    for _ in range(count):
        if attack_type == "Smurf Attack":
            # Smurf attack characteristics
            pkt = IP(src=src_device.IP(), dst=dst_ip) / ICMP(type=8) / ("X" * 5884)  # Padding to make the packet size 6000 bytes
            interval = random.uniform(0.05, 0.39)  # Shorter intervals for attack simulation
        elif attack_type == "Normal ICMP":
            # Randomly choose among typical ICMP types for normal conditions
            icmp_type = random.choice([0, 3])  # echo-request, echo-reply, destination-unreachable
            pkt = IP(src=src_device.IP(), dst=dst_ip) / ICMP(type=icmp_type)
            interval = 0.4  # More relaxed intervals for normal traffic
        else:
        ...
        await asyncio.sleep(interval)  # Respect the specified interval between packets

def gather_traffic_data(net):
    traffic_features = []  # List to store traffic data as feature vectors
    last_icmp_echo_time = {}  # Dictionary to store last ICMP echo request packet time for each host
    ...
    for host in net.hosts:
        ...
        current_time = time.time()
        general_interval = 0.4  # Fixed general interval for UDP and non-ICMP packets

        # Determine the type of traffic
        attack_type = random.choices(
            ['Normal UDP', 'Normal ICMP', 'Smurf Attack'],
            weights=[0.6, 0.25, 0.15],
            k=1)[0]

        if attack_type == "Smurf Attack":
            icmp_type = 8  # ICMP type for echo request (specific to Smurf attacks)
            if src_ip in last_icmp_echo_time:
                icmp_interval = current_time - last_icmp_echo_time[src_ip]
            else:
                icmp_interval = None  # No previous ICMP echo request to calculate an interval from
            last_icmp_echo_time[src_ip] = current_time  # Update the last seen ICMP echo request time
            packet_sizes = [6000] * packet_count  # All packets are 6000 bytes for Smurf attacks
            label = 1  # Mark as an attack
        elif attack_type == "Normal ICMP":
            icmp_type = random.choice([0, 3])  # Randomly choose an ICMP type
            icmp_interval = 0.4
            packet_sizes = [random.randint(40, 1500) for _ in range(packet_count)]
            label = 0  # Mark as normal traffic
        else:
            icmp_type = None
            icmp_interval = None  # No ICMP interval for normal UDP traffic
        ...
        feature = TrafficFeature(src_ip=src_ip, dst_ip=dst_ip, src_port=src_port, dst_port=dst_port,
                                 ...
                                 icmp_interval=icmp_interval, icmp_type=icmp_type, label=label)
...
async def main():
    ...
    task2 = asyncio.create_task(generate_traffic(net, 'h4', '10.255.255.255', random.randint(1024, 65535), 5000, "Smurf Attack"))  # ICMP attack
    task3 = asyncio.create_task(generate_traffic(net, 'h5', '10.255.255.255', random.randint(1024, 65535), 5000, "Smurf Attack"))  # ICMP attack
    task4 = asyncio.create_task(generate_traffic(net, 'h6', '10.0.0.1', random.randint(1024, 65535), 5000, "Normal ICMP"))  # Normal ICMP
