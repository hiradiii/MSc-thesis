...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    ...
    for _ in range(count):
            if attack_type == "SYN_Flood":
                ...
                interval = random.uniform(0.05, 0.39)  # Shorter intervals typical for SYN flood attacks
                ...
                send(pkt, verbose=False)  # Send the packet silently without verbose output
                await asyncio.sleep(interval)  # Use the variable interval for more realistic simulation

            elif attack_type == "Normal TCP":
                tcp_flags = 'R'
                seq_num = random.randint(0, 0xFFFFFFFF)
                window_size = random.randint(1000, 9000)
                interval = 0.4  # Shorter intervals typical for SYN flood attacks
                pkt = IP(src=src_device.IP(), dst=dst_ip) / TCP(sport=random.randint(1024, 65535), dport=dst_port, flags=tcp_flags, seq=seq_num, window=window_size)
                send(pkt, verbose=False)  # Send the packet silently without verbose output
                await asyncio.sleep(interval)  # Use the variable interval for more realistic simulation
                ...
                send(pkt, verbose=False)  # Send the packet silently without verbose output
                await asyncio.sleep(interval)  # Use fixed interval for normal traffic


        # Send the packet silently without verbose output
    send(pkt, verbose=False)
    await asyncio.sleep(interval)  # Use the variable interval for more realistic simulation

def gather_traffic_data(net):
    traffic_features = []  # Store traffic data as feature vectors
    last_syn_time = {}  # Dictionary to store the last SYN packet time for each host
    ...
    for host in net.hosts:
        ...
        current_time = time.time()
        tcp_syn_interval = None  # Initialize to None for all traffic initially

        # Determine the attack type based on a random probability
        attack_type = random.choices(
            ['Normal', 'Normal TCP', 'SYN_Flood'],
            weights=[0.6, 0.25, 0.15],  # Adjust probabilities to include normal TCP traffic
            k=1)[0]

        if attack_type == "SYN_Flood":
            tcp_flags = "S"
            if src_ip in last_syn_time:
                tcp_syn_interval = current_time - last_syn_time[src_ip]  # Calculate interval since last SYN
            last_syn_time[src_ip] = current_time  # Update the last SYN packet time
            label = 1  # Mark as attack traffic
        elif attack_type == "Normal TCP":
            tcp_flags = "R"  # Example flag for normal TCP (could be any other normal flag)
            tcp_syn_interval = 0.4  # Regular interval for normal TCP traffic (fixed value for simplicity)
            label = 0  # Mark as normal traffic
        else:
            tcp_flags = ""  # No flags for other normal traffic
            tcp_syn_interval = None  # No interval necessary for non-TCP traffic
            label = 0  # Normal traffic

        general_interval = 0.4  # Standard interval for all types of traffic
        ...
        feature = TrafficFeature(src_ip=src_ip, dst_ip=dst_ip, src_port=src_port, dst_port=dst_port,
                                 ...
                                 tcp_flags=tcp_flags, tcp_syn_interval=tcp_syn_interval, label=label)
...
async def main():
    ...
    task4 = asyncio.create_task(generate_traffic(net, 'h4', '10.0.0.1', 502, 5000, "Normal TCP"))  # Normal TCP traffic
