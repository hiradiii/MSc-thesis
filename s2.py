...
class DDoSDetector:
    ...
    def detect_ddos(self, labels):
        ...
        for cluster, features in traffic_clusters.items():
            ...
            socket_conditions = [rst_ratio > 0.65, rst_interval_avg < 0.4, len(src_ips) >= 2, active_connections >= 20]
...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    ...
    for _ in range(count):
            if attack_type == "SYN_Flood":
                ...
                interval = random.uniform(0.05, 0.4)  # Shorter intervals typical for SYN flood attacks
...
def gather_traffic_data(net):
    ...
    for host in net.hosts:
        ...
        current_time = time.time()
        tcp_syn_interval = None  # Default to None for non-SYN packets

        # Determine the attack type based on a random probability
        attack_type = "SYN_Flood" if random.random() < 0.15 else "Normal"
        ...
        if attack_type == "SYN_Flood":
            ...
            tcp_flags = ""  # No flags for normal traffic
            label = 0  # Mark as normal traffic

        general_interval = 0.4 if attack_type == "Normal" else random.uniform(0.05, 0.2)  # Shorter intervals for SYN flood
...
async def main():
    ...
    # Traffic generation with specified packet counts
    task1 = asyncio.create_task(generate_traffic(net, 'h1', '10.0.0.1', random.randint(1024, 65535), 6250))  # Normal UDP traffic
    task2 = asyncio.create_task(generate_traffic(net, 'h2', '10.0.0.1', 502, 5000, "SYN_Flood"))  # TCP attack
    task3 = asyncio.create_task(generate_traffic(net, 'h3', '10.0.0.1', 502, 5000, "SYN_Flood"))  # TCP attack
