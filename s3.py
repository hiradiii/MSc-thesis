...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    ...
    for _ in range(count):
        if attack_type == "Smurf Attack":
            # Set the packet size to 6000 bytes for Smurf attacks
...
def gather_traffic_data(net):
    ...
    last_icmp_time = {}  # Separate dictionary to store last ICMP packet time
    ...
    for host in net.hosts:
        ...
        attack_type = "Smurf Attack" if random.random() < 0.15 else None

        if attack_type == "Smurf Attack":
            packet_sizes = [6000] * packet_count  # All packets are 6000 bytes for Smurf attacks
            icmp_type = 8  # Echo-request
            icmp_interval = current_time - last_icmp_time.get(src_ip, current_time)
            last_icmp_time[src_ip] = current_time
            label = 1  # Attack
        else:
