...
class AdaptiveThreshold:
    def __init__(self, initial_threshold, desired_dr=0.90, sensitivity=0.05):
        self.threshold = initial_threshold
        self.desired_dr = desired_dr
        self.sensitivity = sensitivity

    def adjust(self, recent_dr):
        # Adjust the threshold to be more or less sensitive based on the recent detection rate
        if recent_dr < self.desired_dr:
            self.threshold -= self.sensitivity * self.threshold
        else:
            self.threshold += self.sensitivity * self.threshold   
...
class DDoSDetector:
    def __init__(self, k=3, n_init=10):
        ...
        self.rst_threshold = AdaptiveThreshold(0.65)
        self.active_conn_threshold = AdaptiveThreshold(20)
        self.zero_win_threshold = AdaptiveThreshold(55)
    ...
    def detect_ddos(self, labels):
        ...
        for cluster, features in traffic_clusters.items():
            ...
            syn_conditions = [syn_ratio > 0.65, tcp_syn_interval_avg < 0.4, len(src_ips) >= 2]
            smurf_conditions = [icmp_ratio > 0.65, icmp_interval_avg < 0.4, len(src_ips) >= 2]
            socket_conditions = [
                rst_ratio > self.rst_threshold.threshold,
                len(src_ips) >= 2,
                active_connections >= self.active_conn_threshold.threshold,
                zero_window_packets > self.zero_win_threshold.threshold
            ]

            if all(syn_conditions):
                attack_type = "SYN Flood"
                predicted_label = 1
            elif all(smurf_conditions):
                attack_type = "Smurf Attack"
                predicted_label = 1
            elif all(socket_conditions):
        ...
        recent_dr = sum(self.metrics.detection_rates[-10:]) / 10 if len(self.metrics.detection_rates) >= 10 else self.metrics.detection_rates[-1] if self.metrics.detection_rates else 0
        # Adjust thresholds based on recent detection rate
        self.rst_threshold.adjust(recent_dr)
        self.active_conn_threshold.adjust(recent_dr)
        self.zero_win_threshold.adjust(recent_dr)
...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    ...
    for _ in range(count):
        if attack_type == "SYN_Flood":
            ...
            interval = random.uniform(0.05, 0.39)
        ...
        elif attack_type == "Normal TCP":
            tcp_flags = 'F'  # Randomly choosing ACK or SYN for normal operations
            seq_num = random.randint(0, 0xFFFFFFFF)
            window_size = random.randint(1000, 9000)
            interval = 0.4
            pkt = IP(src=src_device.IP(), dst=dst_ip) / TCP(sport=random.randint(1024, 65535), dport=dst_port, flags=tcp_flags, seq=seq_num, window=window_size)
        elif attack_type == "Smurf Attack":
            pkt = IP(src=src_device.IP(), dst=dst_ip) / ICMP(type=8) / ("X" * 5884)
            interval = random.uniform(0.05, 0.39)
        elif attack_type == "Normal ICMP":
            icmp_type = random.choice([0,3])  # Including echo reply (type 0), echo request (type 8), and destination unreachable (type 3)
            pkt = IP(src=src_device.IP(), dst=dst_ip) / ICMP(type=icmp_type)
            interval = 0.4
        elif attack_type == "Socket Stress Attack":
            tcp_flags = "R"
            seq_num = random.randint(0, 0xFFFFFFFF)
            window_size = 0
            interval = random.uniform(0.05, 0.39)
            ...
            # Default to normal UDP traffic if not specified
            interval = 0.4
...
def gather_traffic_data(net):
    ...
    for host in net.hosts:
        ...
        # Determine the attack type based on a random probability
        attack_type = random.choices(
            ['Normal', 'Normal TCP', 'SYN_Flood', 'Smurf Attack', 'Normal ICMP', 'Socket Stress Attack'],
            weights=[0.2, 0.2, 0.05, 0.05, 0.2, 0.05],
            k=1)[0]

        tcp_flags = ""
        label = 0
        general_interval = 0.4
        tcp_syn_interval = None
        icmp_interval = None
        rst_interval = None
        ...
        if attack_type == "SYN_Flood":
            tcp_flags = "S"
            if src_ip in last_syn_time:
                tcp_syn_interval = current_time - last_syn_time[src_ip]
            last_syn_time[src_ip] = current_time
            label = 1
        elif attack_type == "Normal TCP" or attack_type == "Socket Stress Attack":
            tcp_flags = "F" if attack_type == "Normal TCP" else "R"
            if attack_type == "Socket Stress Attack":
                rst_interval = current_time - last_rst_packet_time.get(src_ip, current_time)
            ...
            icmp_type = 8
            if src_ip in last_icmp_echo_time:
                icmp_interval = current_time - last_icmp_echo_time[src_ip]
            last_icmp_echo_time[src_ip] = current_time
            packet_sizes = [6000] * packet_count
            label = 1
        elif attack_type == "Normal ICMP":
            icmp_type = random.choice([0, 3])
            icmp_interval = 0.4
            label = 0
        else:
            label = 0
        ...
        feature = TrafficFeature(src_ip=src_ip, dst_ip=dst_ip, src_port=src_port, dst_port=dst_port,
                                packet_count=packet_count, packet_size=packet_size, interval=general_interval,
                                tcp_flags=tcp_flags, icmp_type=icmp_type, rst_count=0, seq_num=0, window_size=0, label=label,
                                tcp_syn_interval=tcp_syn_interval, icmp_interval=icmp_interval, 
                                rst_interval=rst_interval, connection_start_time=connection_start_time)
...
async def main():
    ...
    task2 = asyncio.create_task(generate_traffic(net, 'h2', '10.0.0.1', 502, 5000, "SYN_Flood"))  # SYN Flood attack
    task3 = asyncio.create_task(generate_traffic(net, 'h3', '10.0.0.1', 502, 5000, "SYN_Flood"))  # SYN Flood attack
    task4 = asyncio.create_task(generate_traffic(net, 'h4', '10.255.255.255', random.randint(1024, 65535), 5000, "Smurf Attack"))  # Smurf Attack
    task5 = asyncio.create_task(generate_traffic(net, 'h5', '10.255.255.255', random.randint(1024, 65535), 5000, "Smurf Attack"))  # Smurf Attack
    task6 = asyncio.create_task(generate_traffic(net, 'h6', '10.0.0.1', 502, 5000, "Socket Stress Attack"))  # Socket Stress Attack
    task7 = asyncio.create_task(generate_traffic(net, 'h7', '10.0.0.1', 502, 5000, "Socket Stress Attack"))  # Socket Stress Attack
    task8 = asyncio.create_task(generate_traffic(net, 'h8', '10.0.0.1', 502, 5000, "Normal TCP"))  # Normal TCP
    task9 = asyncio.create_task(generate_traffic(net, 'h9', '10.0.0.1', random.randint(1024, 65535), 5000, "Normal ICMP"))  # Normal ICMP
