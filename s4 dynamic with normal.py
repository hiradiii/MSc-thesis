...
class DDoSDetector:
    ...
    def detect_ddos(self, labels):
        ...
        for cluster, features in traffic_clusters.items():
            ...
            rst_ratio = sum(1 for f in features if f[9]) / sum(1 for f in features if f[8] or f[9] or f[10]) if sum(1 for f in features if f[8] or f[9] or f[10]) > 0 else 0
            active_connections = sum(1 for f in features if (current_time - f[-2]) < 15)
            zero_window_packets = sum(1 for f in features if f[20] == 0)
...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    ...
    for _ in range(count):
        ...
        if attack_type == "Socket Stress Attack":
            ...
            window_size = 0
            interval = random.uniform(0.05, 0.39)  # Very short intervals for high stress
            ...
            interval = 0.4  # More relaxed interval for normal TCP traffic
            pkt = IP(src=src_device.IP(), dst=dst_ip) / TCP(sport=random.randint(1024, 65535), dport=dst_port, flags=tcp_flags, seq=seq_num, window=window_size)
...
def gather_traffic_data(net):
    ...
    for host in net.hosts:
        ...
        attack_type = random.choices(
            ['None', 'Normal TCP', 'Socket Stress Attack'],
        ...
        feature = TrafficFeature(src_ip=src_ip, dst_ip=dst_ip, src_port=src_port, dst_port=dst_port,
                                 ...
                                 rst_interval=rst_interval, tcp_flags=tcp_flags, window_size=window_size, label=label,
...
async def main():
    ...
    task4 = asyncio.create_task(generate_traffic(net, 'h4', '10.0.0.1', 502, 5000, "Normal TCP"))  # TCP attack
