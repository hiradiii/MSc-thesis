...
async def generate_traffic(net, src_host, dst_ip, dst_port, count, attack_type=None):
    src_device = net.get(src_host)
    for _ in range(count):
        if attack_type == "Socket Stress Attack":
            tcp_flags = "R"
            window_size = 0  # Zero window size for Socket Stress Attack
            interval = random.uniform(0.01, 0.05)
            pkt = IP(src=src_device.IP(), dst=dst_ip) / TCP(sport=random.randint(1024, 65535), dport=dst_port, flags=tcp_flags, seq=random.randint(0, 0xFFFFFFFF), window=window_size)
            send(pkt, verbose=False)
            await asyncio.sleep(interval)
        else:
            interval = 0.4  # Standard interval for normal traffic
